      ...
      fun (password_change_timestamp = Name) ->
          ...
          (group_roles = Name) ->
              {Name, get_user_groups_and_roles(Id, Props, Definitions,
                                               AllPossibleValues)};
          (user_roles = Name) ->
              UserRoles = get_user_roles(Id, Props, Definitions,
                                         AllPossibleValues),
              {Name, UserRoles};
          (roles = Name) ->
              UserRoles = get_user_roles(Id, Props, Definitions,
...
get_user_roles({_, local}, UserProps, Definitions, AllPossibleValues) ->
    menelaus_roles:filter_out_invalid_roles(
     proplists:get_value(roles, UserProps, []),
     Definitions, AllPossibleValues);
get_user_roles({_, external}, UserProps, Definitions, AllPossibleValues) ->
    case ns_config:read_key_fast(use_external_user_roles, true) of
        true ->
            menelaus_roles:filter_out_invalid_roles(
              proplists:get_value(roles, UserProps, []),
              Definitions, AllPossibleValues);
