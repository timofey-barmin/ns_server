...
do_open(Path, TableName, Tries) ->
    case dets:open_file(TableName,
                        [{type, set},
                         ...
                         {file, Path}]) of
        {ok, TableName} ->
            import_records_to_vulcan(TableName),
            ok;
        Error ->
...
import_records_to_vulcan(Table) ->
    ?log_info("Start import of docs from dets ~p", [Table]),
    ...
    lists:foreach(
      fun (Id) ->
              Docs = dets:lookup(Table, Id),
              dets:insert(Table, import_50_docs(Docs))
      end, Ids),
    dets:sync(Table),
    ?log_info("Finished import of docs from dets ~p", [Table]),
    ok.
...
on_replicate_in(Docs, #state{child_module = ChildModule,
                             child_state = ChildState}) ->
    Docs2 = import_50_docs(Docs),
    ChildModule:on_replicate_in(Docs2, ChildState).
...
on_replicate_out(Docs, #state{child_module = ChildModule,
    ...
    Docs2 =
        case cluster_compat_mode:is_cluster_vulcan() of
            true  -> Docs;
            false -> export_to_prevulcan_docs(Docs)
        end,
    ChildModule:on_replicate_out(Docs2, ChildState).
